...
@Component({
  selector: 'app-idcardrequestreport',
  templateUrl: './idcardrequestreport.component.html',
  styleUrls: ['./idcardrequestreport.component.css']
})
export class IdcardrequestreportComponent implements OnInit, AfterViewInit {

  idcardrequestreports: countreport[] = [];
  ...
  ngAfterViewInit(): void {
    this.viewReady = true;
    if (this.idcardrequestreports.length) {
      this.drawCharts();
    }
  }

  async loadStatuses(): Promise<void> {
    this.statuses = await this.rs.getCountReport('idcardrequeststatuses') as any;
  }

  async loadData(): Promise<void> {
    const result = await this.rs.getCountReport('idcardrequestreport');
  ...
  private applyResult(result: countreport[]): void {
    this.idcardrequestreports = result;
  ...
  loadTable(): void {
    this.data = new MatTableDataSource(this.idcardrequestreports);
  }

  drawCharts(): void {
    const labels = this.idcardrequestreports.map(sm => sm.name);
    const counts = this.idcardrequestreports.map(sm => sm.count);
    const percentages = this.idcardrequestreports.map(sm => sm.percentage);
    ...
    new Chart(this.columnchart.nativeElement, {
      ...
      options: {
        responsive: true,
        plugins: {
          title: { display: true, text: 'ID Card Request Report (Bar Chart)' },
          legend: { position: 'top' }
        },
        scales: {
    ...
    new Chart(this.piechart.nativeElement, {
      ...
      options: {
        responsive: true,
        plugins: {
          title: { display: true, text: 'ID Card Request Report (Pie Chart)' }
        }
      }
    });
  }
